...
class RatingDialog @SuppressLint("NonConstantResourceId") constructor(context2: Context) :
    ...
    init {
        ...
        window?.setBackgroundDrawableResource(R.drawable.custom_br_rate)
...
//            rtb.visibility = View.GONE
    ...
    fun setStar(value: Int) {
        ...
        for (i in rates.indices) {
            if (i < value) {
                rates[i].setBackgroundResource(R.drawable.ic_rate_select)
            } else {
                rates[i].setBackgroundResource(R.drawable.ic_rate_unselect)
            }
        }
    }
}